function [user_groups, level_info] = user_grouping_by_level(params, scenario, chan)
% USER_GROUPING_BY_LEVEL  基于随机优先级的用户分组。
%
% 将用户随机分为三个优先级：
...
%
% 然后在MAB选择时根据优先级来约束候选臂的选择策略。

C = params.C;
num_levels = get_level_field(params, "numLevels", 3);

% 计算每个用户的平均信道增益（用于MAB选择时的排序参考）
user_quality = zeros(C, 1);
for c = 1:C
    % 平均信道增益：所有卫星到用户c的平均 |h|^2
